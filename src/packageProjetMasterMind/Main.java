...
public class Main {
	...
	public static void main(String[] args) {
		...
		// int typeJeu = 0; 
		//int entrees.getNCoups() =0; 
		//int entrees.getNPions() =0; 
		// int entrees.getNCouleurs() =0; 
		// int modeJeu =0;
...
// 1 Choix du jeu et du mode
	...
	int combinaisonDefense [] = new int[entrees.getNPions()];
	// combinaison2 en attaque (Mode 2 & 3)
	int combinaisonAttaque []= new int[entrees.getNPions()];
	// Création variable tampon réponse
	int reponse1 [] = new int [entrees.getNPions()];
	...
	if ( entrees.getModeJeu() == 1 || entrees.getModeJeu() == 3) {
		...
		for (int i=0 ; i<combinaisonDefense.length; i++) {
			combinaisonDefense[i]=(int) (Math.random()*entrees.getNCouleurs());
		}
	...
	if (entrees.getModeJeu() == 1) {
		String verdict = new String();
		// Variable réponse corrective de l'ordinateur
		String correctif []= new String[entrees.getNPions()];
		int tour = 0;
		int tourRestant = entrees.getNCoups();
...
// Code de factorisation du test de l'entrée
			...
			TestEntreesJeu entreesJeu = new TestEntreesJeu (reponse1,entrees.getNCouleurs());
			
			//2.1.1.2 comparatif combinaison secrète / entrée => réponse ordinateur
			boleenSiGagne = true;
			
			for (int i=0; i<entrees.getNPions(); i++) {
			...
			//2.1.1.4 test
			tourRestant = entrees.getNCoups() - tour;
	...
	if (  entrees.getModeJeu() == 2) { 
		String verdict = new String();
		verdict = "";
		// Nombre de tours
		int tour = 0;
		// Création tableau 2d réponse (/joueur) j->position pion, k->N coup.
		String tableauReponseJoueur [][]=new String [entrees.getNPions()][entrees.getNCoups()];
		// Création tableau 3d (/ordi)  i->position pion, j->N coup.
		int tableauJeu [][] = new int [entrees.getNPions()][entrees.getNCoups()];	
		...
		while (verdict != "GAGNE!" || verdict != "PERDU!") {
			for (int j=0;j<entrees.getNCoups();j++) {
				...
				for (int i=0;i<entrees.getNPions();i++) {	
					...
					if (tableauReponseJoueur[i][j-1]=="+") {
						tableauJeu[i][j]=(tableauJeu[i][j-1]+entrees.getNCouleurs())/2;
						System.out.println("Pion "+ i + " : " + tableauJeu[i][j]);
						continue;
						}			
					if (tableauReponseJoueur[i][j-1]=="-") {
